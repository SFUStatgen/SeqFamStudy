...
This document discusses the second major step in our work-flow to 
simulate exome-sequencing data in pedigrees ascertained to have four or more relatives affected with lymphoid cancer (the blue box labelled 2).
...
We set the above values for the arguments in the \texttt{sim\_RVped()} function and use the Compute Canada cluster for the simulation. We use an array job on the cluster, with a processor (CPU) to simulate each pedigree. Each pedigree takes time to simulate and it varies from pedigree to pedigree. 
...
- \texttt{\# SBATCH --array= 1:150}- specifies the array of tasks, one for 
each of the 150 pedigrees. 
...
# import study peds
study_peds <- read.table("/Users/jgraham/OneDrive - Simon Fraser University (1sfu)/NirodhaStuff/Data/study_peds.txt", header=TRUE, sep= " ")
...
1. \texttt{FamID}- the identity number of the ascertained pedigre.
