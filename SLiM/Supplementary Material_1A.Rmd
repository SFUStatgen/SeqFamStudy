...
We use the inferred American-admixture demographic model in SLiM but defer discussion of the SLiM 
simulation to section 3 of this document. In the next section, we discuss how to create the recombination map required for the SLiM simulation. 


# Create recombination map for SLiM

To simulate the genome-wide exon-only SNVs from SLiM, we need to supply a recombination map which reads the exon positions in chromosomes. We use the \texttt{create\_SlimMap()} function in the \texttt{SimRVSequence} [@Nieuwoudt2020] R package, as shown in the next code chunk.
...
We use the \texttt{hg\_exons} dataset in the \texttt{SimRVSequence} package. 
The \texttt{hg\_exons} dataset presents the positions of each of the 22 human autosomes, based on 
...
From the \texttt{s_map()} output of \texttt{create\_SlimMap()}, we need three variables to create the recombination map for simulating exon-only data by SLiM: \texttt{recRate}, \texttt{mutRate} and \texttt{endPos}.
...
The next section describes the SLiM model to simulate population-level exon-only SNV sequences.
